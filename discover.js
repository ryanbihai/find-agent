...
find-agent — OceanBus Yellow Pages
  ...
  发布:
    node discover.js publish <name>            发布到黄页
      --tags <tags>                            标签（逗号分隔）
      --desc <description>                     简介
      --help-cmd <command>                     能力描述命令（别人运行它了解你能做什么）
...
async function main() {
  ...
  if (cmd === 'publish') {
    ...
    const helpCmdIndex = args.indexOf('--help-cmd');
    ...
    const helpCommand = helpCmdIndex > -1 ? args[helpCmdIndex + 1] : undefined;

    if (!name || !tags.length) {
      console.error('Usage: node discover.js publish <name> --tags <tag1,tag2> [--desc <description>] [--help-cmd <command>]');
      process.exit(1);
    }

    const result = await ob.l1.yellowPages.publish({ name, tags, description, help_command: helpCommand });
